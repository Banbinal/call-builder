...
ticket depuis `spec.md` ; état actuel : **T0 livré** (scaffold, shell 4 onglets,
invariants encodés dans `CLAUDE.md`).
...
- **Niveau (L1–L4)** : un onglet pédagogique = un dossier `src/levels/l<n>/` exportant
  un composant `L<n>`.
- **Engine** (`src/engine/`) : logique d'appel LLM pure, sans React ni storage, clé
  injectée via `ProviderConfig` (à venir en T1).
